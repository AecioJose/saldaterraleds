//otimização do código e aprimoramento das paginas WEB, funcionalidade de mudar a posição do elemento arrastavel apenas clicando em qualquer lugar do circulo, não necessariamente apenas ao arrastar o elementoarrastavel
...
void setup() {
 ...
 //Wifi manager
  ...
  // Set up mDNS    http://saldaterraleds.local
...
<html lang="pt-br">
    ...
    <body>
        ...
        <script>
          //identifica url e altera: o titulo da pagina, o caminho dos botoes e o indice referente a pagina
          var pageIndice = 0;
          ...
          for (let n = 0; n < 360; n++) {
              ...
              outsideWheel.appendChild(div);
          }
          //


          
          //Elemento Arrastavel e Colisão
          var isDragging = false;

          function calcElementPositionFromMouse(Mouse){
            ...
            const positionX = Mouse.clientX - wheelX;
            const positionY = Mouse.clientY - wheelY;

            const angle = Math.atan2(positionY, positionX);

            const radius = rail.clientWidth / 2;

            const clampedX = wheelCenterX + radius * Math.cos(angle) - draggable_Element.clientWidth / 2;
            const clampedY = wheelCenterY + radius * Math.sin(angle) - draggable_Element.clientHeight / 2;

            draggable_Element.style.left = `${clampedX}px`;
            draggable_Element.style.top = `${clampedY}px`;

            PickColorFromPosition(clampedX, clampedY);
          }

          function calcElementPositionFromTouch(Touch){
          ...
          function PickColorFromPosition(x, y) {
            //PEGA UM ELEMENTO(div.radiusColor) COM BASE NA COORDENADA x E y, PRECISA-SE SOMAR MAIS A DISTÂNCIA DA BORDA DA PÁGINA ATÉ O outsidewheel(left/top) para ter a posição exata dele por conta do position absolute
            ...
            updateSVG_and_SliderColor(r, g, b);
          }

          function updateSVG_and_SliderColor(r, g, b) {
            function calcPorcent(number) {
              ...
              //Atualizando a cor no SVG e salva na memoria temp do navegador
          ...
          insideWheel.addEventListener("mousedown", (e) => {
            e.stopPropagation(); // Impede a propagação do evento para o elementoPAI
          });
          insideWheel.addEventListener("touchstart", (e) => {
            e.stopPropagation(); // Impede a propagação do evento para o elementoPAI
          });
          imglogo.addEventListener("mousedown", (e) => {
              e.stopPropagation(); // Impede a propagação do evento para o elementoPAI
          });
          imglogo.addEventListener("touchstart", (e) => {
              e.stopPropagation(); // Impede a propagação do evento para o elementoPAI
          });
          svglogo.addEventListener("mousedown", (e) => {
              e.stopPropagation(); // Impede a propagação do evento para o elementoPAI
          });
          svglogo.addEventListener("touchstart", (e) => {
              e.stopPropagation(); // Impede a propagação do evento para o elementoPAI
          });
          ...
          outsideWheel.addEventListener("mousedown", (Mouse) => {
            Mouse.preventDefault();
            if (Mouse.target.id === "imglogo") {
              return;
            }
            if (Mouse.target.id === "insideWheel") {
              return;
            }
            if (Mouse.target.id === "svglogo") {
              return;
            }
            console.log("Voce clicou e fora, movendo dragbledelement")
            calcElementPositionFromMouse(Mouse);
            isDragging = true;
          });

          
          outsideWheel.addEventListener("touchstart", (Touch) => {
            if (Touch.target.id === "imglogo") {
              return;
            }
            if (Touch.target.id === "insideWheel") {
              return;
            }
            if (Touch.target.id === "svglogo") {
              return;
            }
            console.log("Voce clicou e fora, movendo dragbledelement")
            calcElementPositionFromTouch(Touch)
            isDragging = true;
          });
          ...
          //Slider
          updateSVG_and_SliderColor(250, 250, 255); //Inicia o Slider com a cor branca(quase branca)
          var slider = document.getElementById("intensityInput");
          var output = document.getElementById("slider-value");
          output.innerHTML = slider.value;
          
          slider.oninput = function () {
            //ao mecher no slider ele altera no output
          ...
          //Pegar cor das bolinhas(as 12 abaixo do circulo) e atualiza no slider e na logo
          function pickcolor(me) {
              ...
              if (rgbaMatch) {
                  ...
                  updateSVG_and_SliderColor(red, green, blue);
              }
          }

        
          //Pega a cor da logo e enviar para o server via JSON
